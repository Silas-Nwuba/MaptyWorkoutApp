...
class App {
    ...
    _loadMap(position) {
        ...
        map = L.map("map").setView(coords, this.workoutZoom);
        const tile = L.tileLayer("http://{s}.google.com/vt/lyrs=m&x={x}&y={y}&z={z}", {
            maxZoom: 15,
            subdomains: [
                "mt0",
                "mt1",
    ...
    _readerWorkoutOnMap(workout) {
        //display maker
        marker = new L.marker(workout.coords, {
            riseOnHover: true
    ...
    _movePopupMap(e) {
        ...
        // //prettier-ignore
        map.setView(workout.coords, this.workoutZoom, {
            animate: true,
            pan: {
                duration: 1
            }
    ...
    _deleteWorkoutFromUI(e) {
        deleteAllWorkout.addEventListener("click", (e)=>{
            if (e.target) this.clearLocalStorage();
        });
    }
}
//confirmation modal
